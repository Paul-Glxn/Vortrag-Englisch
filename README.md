...
<head>
  ...
  <title>Staten Island — Digital Poster (Final Deluxe)</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    header .emoji{font-size:48px;filter: drop-shadow(0 5px 12px rgba(34,211,238,.25));}
    ...
    .card{grid-column: span 12;background: rgba(255,255,255,.03);border:1px solid rgba(148,163,184,.15);border-radius: var(--radius);padding:22px;box-shadow: var(--shadow);position:relative}
    .card h2{margin:0 0 14px 0;font-size: clamp(20px, 3vw, 28px);display:flex;align-items:center;gap:10px}
    .card::before{content:"";position:absolute;top:0;left:0;right:0;height:6px;border-top-left-radius:var(--radius);border-top-right-radius:var(--radius);background:linear-gradient(90deg,var(--accent),var(--accent-2))}

    .sentence{background:rgba(15,23,42,.55); border:1px solid rgba(148,163,184,.2); padding:14px; border-radius:14px; display:flex; flex-direction:column; gap:8px;margin-bottom:12px}
    ...
    .translation{margin-top:6px;background:var(--translate-bg);padding:10px;border-radius:10px;border:1px solid rgba(148,163,184,.2);display:none;font-size:0.95rem;color:#e2e8f0;position:relative}
    .translation::before{content:"🇩🇪";position:absolute;top:8px;right:12px;opacity:0.8}
    .translation.open{display:block}

    iframe{box-shadow:var(--shadow);border-radius:12px}

    footer{margin-top:40px;text-align:center;font-size:0.9rem;color:var(--muted)}
  ...
  <div class="wrap">
    ...
    <div class="grid">
      ...
      <section class="card">
        ...
        <iframe width="100%" height="280" style="border:0;margin-top:14px" loading="lazy" src="https://www.openstreetmap.org/export/embed.html?bbox=-74.33%2C40.47%2C-74.02%2C40.67&amp;layer=mapnik&amp;marker=40.58%2C-74.14"></iframe>
      </section>
    </div>

    <footer>
      <p><em>This poster was created to present Staten Island bilingually, combining geography, history, and language practice.</em></p>
      <p><em>Dieses Poster wurde zweisprachig gestaltet, um Geografie, Geschichte und Sprachpraxis zu verbinden.</em></p>
    </footer>
  </div>
